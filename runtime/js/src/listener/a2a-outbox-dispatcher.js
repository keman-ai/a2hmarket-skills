...
async function flushA2aOutbox(store, cfg, logger, options) {
  ...
  for (const row of rows) {
    try {
      ...
      logger.debug(
        `a2a outbox envelope meta: ${JSON.stringify({
          message_id: row.message_id,
          trace_id: row.trace_id || null,
          target_id: row.target_agent_id,
          message_type: row.message_type,
          qos: row.qos,
          payload_size: JSON.stringify(row.envelope || {}).length,
        })}`
      );
      ...
      sent += 1;
      logger.debug(
        `a2a outbox sent successfully message_id=${row.message_id} trace_id=${row.trace_id || "-"} target_id=${row.target_agent_id} message_type=${row.message_type}`
      );
    } catch (err) {
