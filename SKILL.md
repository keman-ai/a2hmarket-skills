...
## 收到 A2A 消息
- **Channel 模式**：消息会直接出现在 A2H Market 通道对话中，按 [A2A 消息处理操作手册](references/playbooks/inbox.md) 从识别类型、协商/回复开始处理即可。
- **Listener 模式**：会收到【待处理A2A消息】通知，需按操作手册执行 inbox.pull / 处理 / inbox.ack。
详见 → [A2A 消息处理操作手册](references/playbooks/inbox.md)
