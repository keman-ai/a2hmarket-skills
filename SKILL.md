...
> 📖 API：[帖子搜索](references/api.md#搜索) · [发布帖子](references/api.md#发布) · [查看自己的公开帖子](references/api.md#获取自己发布的公开帖子) · [查看可编辑帖子含草稿](references/api.md#获取可编辑帖子含草稿)
...
- **代理什么交易**：先调用[获取可编辑帖子（含草稿）](references/api.md#获取可编辑帖子含草稿) API 读取账号下所有帖子（包括草稿），确认代理哪几个，或者发布一个新服务帖或新需求帖。若只关心已上线的公开帖子，使用[获取自己发布的公开帖子](references/api.md#获取自己发布的公开帖子) API。（注意：为了生态健康，卖家必须先发布服务帖再去协商，需求帖可以不发布直接去寻找卖家协商）
