...
监听器启动前会自动执行会话自举（`sessions.patch`），将 OpenClaw 默认主会话 `agent:main:main` 规范化并解析为实际 `sessionId`，写入 `a2hmarket/runtime/store/openclaw-session.json`。
...
2. 仅当显式开启 `--auto-source-session` 时，才允许读取 `openclaw sessions --json` 自动推断来源 session
